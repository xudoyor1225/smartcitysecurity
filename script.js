...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const denyBtn = document.getElementById('denyMusicBtn');
  ...
  } else if (musicPermission === 'allowed') {
    // Try to auto-play if allowed (might still be blocked by browser until interaction)
    // We'll try to play on the first click anywhere if autoplay fails
    document.addEventListener('click', () => {
      if (window.toggleMusic && document.getElementById('bgMusic').paused) {
        window.toggleMusic();
      }
    }, { once: true });
  }
  ...
  if (denyBtn) {
    denyBtn.addEventListener('click', () => {
      localStorage.setItem('musicPermission', 'denied');
      if (musicModal) musicModal.classList.remove('active');
    });
  }
});
